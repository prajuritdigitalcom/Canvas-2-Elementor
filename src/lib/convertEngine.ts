...
5. Konversi setiap class utility Tailwind ke CSS asli di dalam satu blok <style> di <head>, dengan strategi HYBRID:
   ...
   b. Jika sebuah kombinasi class hanya dipakai SEKALI di seluruh dokumen (styling unik untuk 1 elemen spesifik) -> tulis langsung sebagai inline style="..." pada elemen tersebut, JANGAN buat class CSS baru untuk ini.
      ...
      ATURAN NAMA PROPERTI CSS (WAJIB DIPATUHI PERSIS): Nama properti CSS HARUS ditulis sesuai spesifikasi resmi CSS, tidak boleh dibalik urutan katanya atau dikarang. Contoh yang SERING SALAH dan HARUS DIHINDARI:
         - "align-items" (BENAR) vs "items-align" (SALAH, tidak valid)
         - "align-content" (BENAR) vs "content-align" (SALAH, tidak valid)
         - "align-self" (BENAR) vs "self-align" (SALAH, tidak valid)
         - "justify-content" (BENAR) vs "content-justify" (SALAH, tidak valid)
         - "justify-items" (BENAR) vs "items-justify" (SALAH, tidak valid)
         - "justify-self" (BENAR) vs "self-justify" (SALAH, tidak valid)
      Sebelum output dianggap selesai, WAJIB periksa ulang setiap baris di dalam blok <style> maupun di setiap atribut inline style="..." — pastikan tidak ada satupun nama properti CSS yang tidak dikenali/tidak valid menurut spesifikasi CSS resmi (bukan hanya untuk properti flexbox di atas, tapi untuk SEMUA properti CSS yang ditulis).
